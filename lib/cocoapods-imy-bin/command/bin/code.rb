      ...
      class Code < Bin
        ...
        def download_source(name)
          ...
          if find_dependency.external_source == nil
            specSet = config.sources_manager.search(find_dependency)
            specs = all_paths_from_set(specSet)
            specs.each do |tmpSpec|
              if tmpSpec.to_s.include?find_dependency.requirement.requirements.first.last.version.to_s
                spec = Specification.from_file(tmpSpec)
                break
              end
            end
          elsif
